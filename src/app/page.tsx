...
export default async function HomePage() {
  ...
  return (
    <main className="mx-auto max-w-3xl p-6">
      <h1 className="text-3xl font-bold mb-6">Weeds.dk  Cannabis News</h1>
      <ul className="space-y-4">
        {news.map((item) => (
          <li key={item.link} className="border rounded p-4 hover:bg-gray-50">
            <div className="text-sm text-gray-500">{item.source}  {formatDate(item.publishedAt)}</div>
            <Link href={item.link} target="_blank" className="text-lg font-semibold text-blue-600 hover:underline">
              {item.title}
            </Link>
            {item.summary ? (
